...
// ── Subdomain routing — must come BEFORE static middleware ─────────────────
app.use((req, res, next) => {
  const host = req.hostname || '';
  if (host.startsWith('custodian.')) {
    return res.sendFile(require('path').join(__dirname, 'public', 'custodian.html'));
  }
  next();
});
  ...
  "industry_trend": "Current industry trend or recent focus area for this chapter"
    ...
    const aiRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 3000,
        ...
        messages: [{
          role: 'user',
          content: `Search for recent SIRE 2.0 inspection findings, deficiencies, and common observations for ${vessel_type || 'LPG Gas Carrier'} vessels specifically in the area of "${chapter_title}". Look for OCIMF reports, maritime authority publications, industry safety alerts, and P&I club circulars about common SIRE findings. Then provide a structured analysis.\n\n${prompt}`
        }]
      })
    });
    const aiData = await aiRes.json();
    ...
    const parsed = jsonMatch ? JSON.parse(jsonMatch[0]) : { findings: [], chapter_risk_areas: [], industry_trend: text.substring(0, 500) };
    res.json(parsed);
  } catch(e) {
...
- Mooring, anchor → "Mooring"`;
  ...
  const aiRes = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 800, messages: [{ role: 'user', content: msgContent }] })
  });
  const aiData = await aiRes.json();
